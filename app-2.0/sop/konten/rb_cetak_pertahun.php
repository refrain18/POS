...
if ( !$thn || (date_parse($thn)['error_count'] != 0) ) {
...
$query = "SELECT 
    ...
    (SELECT COUNT(hasil_rundown) FROM sop b WHERE YEAR(b.tanggal) = YEAR(a.tanggal) AND b.hasil_rundown = 'Terpenuhi') as total_completed, 
    (SELECT COUNT(hasil_rundown) FROM sop c WHERE YEAR(c.tanggal) = YEAR(a.tanggal) AND c.hasil_rundown != 'Terpenuhi') as total_incompleted 
  FROM sop a WHERE YEAR(a.tanggal) = '$thn' GROUP BY YEAR(a.tanggal);
";
...
$query = "SELECT 
  ...
  FROM  sop a WHERE YEAR(a.tanggal) = '$thn' GROUP BY MONTHNAME(a.tanggal) ORDER BY a.tanggal;
";
